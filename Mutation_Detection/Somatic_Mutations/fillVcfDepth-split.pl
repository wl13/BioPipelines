...
while (<$list_fh>)
{
    ...
    $sample_libs{$sample_id}->{$library_id} = $file;
}
print STDERR "done!\n";
...
fillVcfDepth(\%sample_libs, $vcf_file);
print STDERR "done!\n";
...
{
    my ($rh_depth, $sample_id, $library_id, $in, $region) = @_;
...
EOF
        ...
        if (!$region_parsed{$CHROM}) {
            undef(%read_counts);
            
            for my $sample (sort keys %{$rh_sample_infos})
            {
                for my $lib (sort keys %{$rh_sample_infos->{$sample}})
                {
                    my $lib_file = $rh_sample_infos->{$sample}->{$lib};
                    
                    parse_depth_file(\%read_counts, $sample, $lib, $lib_file, $CHROM);
                }
            }
            
            $region_parsed{$CHROM} = 1;
        }
        
        next unless( $read_counts{total}->{"$CHROM:$POS"} ); ## exclude records with no DPA info available
        ...
        for (my $i=0; $i<@SAMPLES; $i++)
        {
            ...
            my %detail_dps = ();
            my @libraries  = sort keys %{$read_counts{$sample_id}->{"$CHROM:$POS"}};
            ...
            for (my $k=0; $k<@libraries; $k++)
            {
                ...
                for (my $j=0; $j<@vars; $j++)
                {
                    ...
                    my $count_plus  = $read_counts{$sample_id}->{"$CHROM:$POS"}->{$lib_id}->{$base}->{plus}  ?
                                      $read_counts{$sample_id}->{"$CHROM:$POS"}->{$lib_id}->{$base}->{plus}  : 0;
                    my $count_minus = $read_counts{$sample_id}->{"$CHROM:$POS"}->{$lib_id}->{$base}->{minus} ?
                                      $read_counts{$sample_id}->{"$CHROM:$POS"}->{$lib_id}->{$base}->{minus} : 0;
