...
* **[FATAL level: most likely false positive calls]**
        ...
        4. UG_misalign: SNV only called by UnifideGenotyper in slippage/tandem region with nearby INDEL (UG-only + near_indel + trf_predict);
		
        5. pre_exist: mutation allele is found in pre-existing variants (require a list of pre-existing alleles that should not be taken as mutations);
		
        6. unexpected_cooccur: mutation co-occurred preferentially in non-closely related samples rather than closely related samples (based on the experimental design on the kinship of samples)
...
* **[WARN level: need cautious]**
        ...
        10. lack_reliable_reads: SNV mutation allele is only supported by non-reliable reads (e.g., no read-support after excluding the clipped ones);
...
> Check supporting reads of  mutation allele. This script checks whether a mutation allele was supported by reliable reads by excluding reads failed various thresholds here, multiple thresholds will be applied together if specified
...
* Example codes:
		...
		check_bam_supports.pl --input combined/samples.fq19.snp.mut.combined.vcf \
		    --bam-list bam_files.list --samtools "-F 3844" --max-clip-ratio 0.1 \
		    > combined/samples.fq19.snp.mut.combined.bam_infos.csv
